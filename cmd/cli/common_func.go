...
func getShovelList(address, tickId string) ([]*mrc20_service.MintPin, error) {
	...
	count, _ = strconv.ParseInt(info.Qual.Count, 10, 64)
	if count == 0 {
		return nil, nil
	}
	list, total, err := man.DbAdapter.GetShovelListByAddress(address, tickId, info.Qual.Creator, lv, path, query, key, operator, value, 0, 1000)
	if err != nil {
		return nil, err
	}
	if count > total {
